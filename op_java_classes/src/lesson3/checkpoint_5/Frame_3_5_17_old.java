...
public class Frame_3_5_17_old extends JPanel {
	...
	void killAll() throws Exception {
		while (getBrickQuadrant() != null) {
			System.out.println("Kill all. Mooving to: " + getBrickQuadrant()[0] + getBrickQuadrant()[1]);
			moveToQuadrant(getBrickQuadrant()[0], getBrickQuadrant()[1]);
		}
	}
	...
	int[] getBrickQuadrant() {
		...
		int[] tankPos = getQuadrant(tankX,tankY);

		if (tankPos[1] < MAX_QUADRANTS / 2) {
			runDirection = 1;
		}

		return fieldRunner(runDirection);
	}

	int[] fieldRunner(int runDirection) {

		int [] res = null;
		
		for (int y = MIN_QUADRANTS; y <= MAX_QUADRANTS; y++) {
			if (runDirection == 1) {
				System.out.println("X axel mooving from " + MIN_QUADRANTS + " to " + MAX_QUADRANTS);
				for (int x = MIN_QUADRANTS; x <= MAX_QUADRANTS; x++) {
				...
				System.out.println("X axel mooving from " + MAX_QUADRANTS + " to " + MIN_QUADRANTS);
				for (int x = MAX_QUADRANTS; x >= MIN_QUADRANTS; x--) {
					if (battleField[y][x].equals("B")) {
						res = new int[] { y, x };
					}
				}
	...
	public static void main(String[] args) throws Exception {
		Frame_3_5_17_old bf = new Frame_3_5_17_old();
		bf.runTheGame();
	}

	public Frame_3_5_17_old() throws Exception {
